...
namespace Dynamite.DynamiteEngine.Players.ConcreteProduct
{
    class ZombieCharacter : Player
    {
        ...
        public ZombieCharacter(MapProduct _map, int positionX, int positionY)
        {
            this.map = _map;
            this.Position = new Vector2(positionX, positionY);
            CharacterImg = new Sprite2D(new Vector2(positionX, positionY), new Vector2(48, 64),
                PlayerName, "/Characters/Zombie/PNG/Poses/character_zombie_walk1");
        }

        public ZombieCharacter(string playerName, MapProduct _map, int positionX, int positionY)
        {
