...
namespace Dynamite.DynamiteEngine.Players.ConcreteProduct
{
    class HumanMaleCharacter : Player
    {
        ...
        public HumanMaleCharacter(MapProduct _map, int positionX, int positionY)
        {
            this.map = _map;
            this.Position = new Vector2(positionX, positionY);
            CharacterImg = new Sprite2D(new Vector2(positionX, positionY), new Vector2(48, 64),
                PlayerName, "/Characters/Male person/PNG/Poses/character_malePerson_walk1");
        }

        public HumanMaleCharacter(string playerName, MapProduct _map, int positionX, int positionY)
        {
