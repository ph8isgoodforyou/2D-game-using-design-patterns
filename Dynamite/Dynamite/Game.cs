        ...
        public void BonusLogic(Player player)
        {
            ...
            for (int i = 0; i < player.BonusSlot.Length; i++)
            {
                if (player.BonusSlot[i] != BonusType.None)
                {
                    ...
                    {
                        if (player.BonusSlot[i] == BonusType.SpeedBoost)
                        {
                            player.Speed /= 2;
                        }
            ...
            if (this.world.MapGrid[player.CasePosition[0], player.CasePosition[1]].BonusHere != null)
            {
                ...
                if (freeSlot != -1)
                {
                    ...
                    {
                        case BonusType.PowerBomb:
                            player.BonusSlot[freeSlot] = BonusType.PowerBomb;
                            player.BonusTimer[freeSlot] = 5000;
                            break;
                        case BonusType.SpeedBoost:
                            player.BonusSlot[freeSlot] = BonusType.SpeedBoost;
                            player.Speed *= 2;
                            player.BonusTimer[freeSlot] = 5000;
                            break;
                        case BonusType.Desamorce:
                            player.BonusSlot[freeSlot] = BonusType.Desamorce;
                            player.BonusTimer[freeSlot] = 10000;
                            break;
                        case BonusType.Armor:
                            player.BonusSlot[freeSlot] = BonusType.Armor;
                            player.BonusTimer[freeSlot] = 5000;
                            break;
                        default:
                            break;
